...
// Go to previous question, as long as the game has not ended yet
// # Handler
game.beginPreviousQuestion = function (req, res) {
  console.log('INFO - Beginning previous question');
  
  // Switch to last question if game had ended
  if (game.currentStatus === 'ENDED') {
    game.questions.count({}, function (err, count) {
      game.switchToQuestion(count, function (err) {
        if (err) {
          return game.resetGame(req, res);
        } else {
          game.sendStatus();
          return res.status(200).end();
        }
  ...
  var previousNumber = game.currentQuestion ? game.currentQuestion.number - 1 : 1;

  game.switchToQuestion(previousNumber, function (err) {
    if (err) {
      console.log('INFO - No question found, back to beginning');
      return game.resetGame(req, res);
    }
...
// Get the highest score (so not optimized)
// # Handler
game.getHighestScore = function (req, res) {
  var highestScore = 0;

  game.questions.find({}, function (err, questions) {
    game.players.find({}, function (err, players) {
      players.forEach(function (player) {
        ...
        questions.forEach(function (question) {
          if (player.answers) {
            answer = player.answers[question.number.toString()];
          } else {
            answer = "unaccepted";
          }
          if (question['answer' + answer + 'Valid']) { score += 1 }
        });

        highestScore = Math.max(score, highestScore);
      });

      return res.status(200).json({ highestScore: highestScore });
    });
  });
};
...
// Switch to previous state in line
// # Handler
game.previousState = function (req, res) {
  switch (game.currentStatus) {
    case 'NOT_STARTED':
      return res.status(200).end();
      break;
    ...
    case 'SHOW_RESULT':
    case 'ENDED':
      return game.beginPreviousQuestion(req, res);
      break;
  }
};
